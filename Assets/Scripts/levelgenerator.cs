    ...
    void FixedUpdate()
    {
        ...
        if (randomNumber > 0.975f - difficulty * 0.0001f)
        {
            ...
            if (randomNumber > 0.98f)
            {
                ...
                ppipe.transform.localScale = new Vector3(0.7f+1.5f*wallRandom,0.5f + heightRandom, 0.7f + 1.5f*wallRandom);
                ppipe.transform.rotation = Quaternion.identity;
                ppipe.transform.position = new Vector3(Random.Range(-5, 5f), 0.55f + heightRandom, 36);
                Rigidbody m_Rigidbody = ppipe.GetComponent<Rigidbody>();
                m_Rigidbody.velocity = new Vector3(0, 0, -15f);
            }
